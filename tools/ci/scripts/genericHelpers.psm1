...
Write-Host "`n`Executing Unity Player Method"
...
function BuildUnityProject
{
    ...
    $Method = "Microsoft.MixedReality.BuildTools.Unity.UnityPlayerBuildTools.StartCommandLineBuild"
    $UpdatedOutDirExt = "Build_" + $OutDirExt;
    ExecuteMethod -Method $Method -ProjectPath $ProjectPath -SceneList $SceneList -EditorPath $EditorPath -Platform $Platform -Arch $Arch -ScriptingBackend $ScriptingBackend -UnityArgs $UnityArgs -Define $Define -BuildArtifactStagingDirectory $BuildArtifactStagingDirectory -UnityCacheServerAddress $UnityCacheServerAddress -OutDirExt $UpdatedOutDirExt -Succeeded $Succeeded
}
...
function ConfirmEditorCompiles
{
    ...
    $Method = "Microsoft.MixedReality.BuildTools.Unity.UnityPlayerBuildTools.ConfirmEditorCompiles"
    $UpdatedOutDirExt = "EditorTest_" + $OutDirExt;
    ExecuteMethod -Method $Method -ProjectPath $ProjectPath -SceneList $SceneList -EditorPath $EditorPath -Platform $Platform -Arch $Arch -ScriptingBackend $ScriptingBackend -UnityArgs $UnityArgs -Define $Define -BuildArtifactStagingDirectory $BuildArtifactStagingDirectory -UnityCacheServerAddress $UnityCacheServerAddress -OutDirExt $UpdatedOutDirExt -Succeeded $Succeeded
}